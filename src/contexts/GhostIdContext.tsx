...
export const GhostIdProvider = ({ children }: { children: ReactNode }) => {
	...
	const { isConnected } = useSocket(nest)
	...
	const registerWithServer = useCallback(async () => {
		...
		try {
			...
			const result = await registerUser(walletKey, nest)
	...
	const requestGhostId = useCallback(async () => {
		...
		if (!isConnected) {
			...
			getSocket() || initializeSocket(nest)
			setTimeout(() => {
		...
		try {
			const socketInstance = getSocket() || initializeSocket(nest)
