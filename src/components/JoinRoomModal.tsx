...
import { Copy, Ghost, Key, X, ArrowRight, Scan, Wallet, Loader2, CheckCircle2, AlertTriangle, User } from "lucide-react"
...
const JoinRoomModal: React.FC<JoinRoomModalProps> = ({ isOpen, onClose, onJoinSuccess, onJoinPending }) => {
  ...
  const handleRoomIdSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await requestJoinRoom(roomToJoin, currentNickname)
      if (result.success) {
        if (result.status === "joined" && result.roomData) {
          // FIX: Pass the nickname in the roomData object here
          const roomDataWithNickname = { ...result.roomData, nickname: currentNickname }
          onJoinSuccess(roomDataWithNickname, currentNickname)
        } else if (result.status === "pending" && result.roomData) {
          // FIX: Pass the nickname in the roomInfo object here
          const roomInfoWithNickname = { ...result.roomData, nickname: currentNickname }
          onJoinPending(roomInfoWithNickname, currentNickname)
        }
        onClose()
      } else {
  ...
  const handleNftSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const verifyData = await verifyNftAccessPass(API_URL, selectedClientNft.nftIdentifier, publicKey.toString())
      ...
      const joinResult = await requestJoinRoom(verifiedRoomId, currentNickname)

      if (joinResult.success) {
        if (joinResult.status === "joined" && joinResult.roomData) {
          // FIX: Pass the nickname in the roomData object here
          const roomDataWithNickname = { ...joinResult.roomData, nickname: currentNickname }
          onJoinSuccess(roomDataWithNickname, currentNickname)
        } else if (joinResult.status === "pending" && joinResult.roomData) {
          // FIX: Pass the nickname in the roomInfo object here
          const roomInfoWithNickname = { ...joinResult.roomData, nickname: currentNickname }
          onJoinPending(roomInfoWithNickname, currentNickname)
        }
        onClose()
      } else {
  ...
  const handleNftSelectionConfirm = () => {
    setIsNftSelectionModalOpen(false)
  }

  const handleNftSelectionClose = () => {
    setIsNftSelectionModalOpen(false)
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          ...
          <>
            <div className="mb-4">
              <label
                ...
                className="block text-sm mb-2.5 text-gray-300 flex items-center tracking-tight"
              >
                <User size={15} className="mr-1.5 text-[#FF4D00]" /> Your Nickname{" "}
                <span className="text-[#FF4D00] ml-1">*</span>
              </label>
              <input
                id="nicknameInput"
                type="text"
              ...
              <form onSubmit={handleRoomIdSubmit} className="space-y-5">
                <div>
                  <label
                    ...
                    className="block text-sm mb-2.5 text-gray-300 flex items-center tracking-tight"
                  >
                    <Copy size={15} className="mr-1.5 text-[#FF4D00]" /> Room ID{" "}
                    <span className="text-[#FF4D00] ml-1">*</span>
                  </label>
                    ...
                    {clientNfts.length === 0 && !isFetchingClientNfts ? (
                      <div className="border border-[#333333] rounded-xl p-5 text-center">
                        ...
                        <p className="text-xs text-gray-400 mb-5">
                          Click the button to find NFT passes in your wallet.
                        </p>
                        <button
                          type="button"
                            ...
                            <div className="border-2 border-[#FF4D00] rounded-xl p-4 bg-[#FF4D00]/10">
                              <div className="flex items-center">
                                <div className="w-16 h-16 rounded-lg overflow-hidden mr-4 flex-shrink-0 bg-[#222]">
                                  <img
                                    src={
                                      selectedClientNft.imageUrl ||
                                      `https://placehold.co/64x64/111111/555555?text=${selectedClientNft.name ? selectedClientNft.name.charAt(0) : "N"}`
                                    }
                                    alt={selectedClientNft.name || "NFT Image"}
                                    className="w-full h-full object-cover"
                                    onError={(e) =>
                                      (e.currentTarget.src = `https://placehold.co/64x64/111111/555555?text=${selectedClientNft.name ? selectedClientNft.name.charAt(0) : "N"}`)
                                    }
                                  />
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="text-base font-medium text-white mb-1">
                                    {selectedClientNft.name || "Unnamed NFT"}
                                  </p>
                                  <p className="text-sm text-gray-300">
                                    {selectedClientNft.nftIdentifier.substring(0, 8)}...
                                    {selectedClientNft.nftIdentifier.substring(
                                      selectedClientNft.nftIdentifier.length - 8,
                                    )}
                                  </p>
                                </div>
                                <button
                                  type="button"
                                  onClick={handleOpenNftSelection}
                          ...
                          <>
                            ...
                            <div className="space-y-2.5 border border-[#333333] rounded-xl p-3">
                                ...
                                >
                                  <div className="w-10 h-10 rounded-md overflow-hidden mr-3 flex-shrink-0 bg-[#222]">
                                    <img
                                      src={
                                        nft.imageUrl ||
                                        `https://placehold.co/60x60/111111/555555?text=${nft.name ? nft.name.charAt(0) : "N"}`
                                      }
                              ...
                              {clientNfts.length > 3 && (
                                ...
                                >
                                  View All {clientNfts.length} NFTs
                                </button>
                              )}
                            </div>
                          </>
      ...
      <NftSelectionModal
        ...
        onClose={handleNftSelectionClose}
        nfts={clientNfts}
        selectedNft={selectedClientNft}
        onSelectNft={handleSelectClientNft}
        onConfirm={handleNftSelectionConfirm}
      />
    </div>
  )
}
