    ...
    try {
      ...
      const sendChunks = async () => {
        if (!messageId) {
          ...
          resolve({
            success: false,
            error: "Failed to initialize large message",
          });
          return;
        }
        ...
        try {
          ...
          resolve({
            success: false,
            error: "Error sending chunks: " + (err as Error).message,
          });
        }
      };
    } catch (err) {
      resolve({
        success: false,
        error: "Error preparing data: " + (err as Error).message,
      });
    }
  });
};
...
export const requestFileDownload = (
  ...
  return new Promise((resolve) => {
    ...
    const handleFileDownloaded = (data: { fileId: string; fileName: string; fileData: ArrayBuffer }) => {
      socket.off("fileDownloaded", handleFileDownloaded);
      socket.off("error", handleError);
    ...
    socket.on("fileDownloaded", handleFileDownloaded);
    socket.on("error", handleError);

    socket.emit("requestFileDownload", { fileId });
  });
};
