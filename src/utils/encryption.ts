...
/**
 * Encrypts a raw ArrayBuffer (like a file) using a provided AES key.
 * @param data The ArrayBuffer to encrypt.
 * @param aesKey The symmetric CryptoKey to use for encryption.
 * @returns A promise that resolves to the encrypted data, initialization vector, and auth tag.
 */
export async function encryptBuffer(
  data: ArrayBuffer,
  aesKey: CryptoKey,
): Promise<{ iv: Uint8Array; encryptedData: ArrayBuffer; authTag: Uint8Array }> {
  const iv = window.crypto.getRandomValues(new Uint8Array(12))

  // The result of AES-GCM encryption includes the ciphertext and the auth tag appended at the end.
  const fullEncryptedBuffer = await window.crypto.subtle.encrypt({ name: "AES-GCM", iv }, aesKey, data)

  // The auth tag for AES-GCM is typically 128 bits (16 bytes). We need to separate it.
  const tagLengthInBytes = 16
  if (fullEncryptedBuffer.byteLength < tagLengthInBytes) {
    throw new Error("Encrypted data is too short to contain an auth tag.")
  }

  const encryptedData = fullEncryptedBuffer.slice(0, fullEncryptedBuffer.byteLength - tagLengthInBytes)
  const authTagBuffer = fullEncryptedBuffer.slice(fullEncryptedBuffer.byteLength - tagLengthInBytes)

  return { iv, encryptedData, authTag: new Uint8Array(authTagBuffer) }
}

/**
 * Decrypts an ArrayBuffer using a provided AES key, IV, and auth tag.
 * @param encryptedData The encrypted data to decrypt.
 * @param aesKey The symmetric CryptoKey to use for decryption.
 * @param iv The base64 encoded initialization vector.
 * @param authTag The raw authentication tag.
...
export async function decryptBuffer(
  ...
  // Re-combine the encrypted data and the authentication tag
  ...
  return window.crypto.subtle.decrypt({ name: "AES-GCM", iv: ivBuffer }, aesKey, combined.buffer)
}

/**
 * Decrypts a base64 encoded AES key using the recipient's private RSA key.
 * @param encryptedKey The base64 encoded, RSA-encrypted AES key.
 * @param privateKey The recipient's private CryptoKey.
 * @returns A promise that resolves to the decrypted AES CryptoKey.
 */
export async function decryptAesKey(encryptedKey: string, privateKey: CryptoKey): Promise<CryptoKey> {
  ...
  const aesKeyData = await window.crypto.subtle.decrypt({ name: "RSA-OAEP" }, privateKey, encryptedKeyBuffer)
  // Import the raw key data to create a usable CryptoKey object
